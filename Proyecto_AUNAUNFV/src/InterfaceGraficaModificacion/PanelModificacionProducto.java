...
public class PanelModificacionProducto extends JPanel  
{
    private JLabel titulo,cod,nom,des,pv,can,est,labelimagen,mensaje2,txtCod;
    private JTextField txtNom,txtDes,txtPv,txtCan,txtEst;
    private JButton btnModificar,btnCancelar,btnCargarFoto;
    ...
    public PanelModificacionProducto(ProductoBEAN paramProducto)
    {
    ...
    public void llenarCampos(){
        ...
        if(paramProducto!=null){
            txtCod.setText(paramProducto.getCodProducto());
            txtNom.setText(paramProducto.getNombre());
            txtDes.setText(paramProducto.getDescripcion());
            txtPv.setText(paramProducto.getPrecioVenta() + "");
            txtCan.setText(paramProducto.getCantidad() + "");
            txtEst.setText(paramProducto.getEstado() + "");
            
            try{
                byte[] bi = paramProducto.getImagen();
                InputStream is=new ByteArrayInputStream(bi);
                BufferedImage image=ImageIO.read(is);
                ImageIcon foto=new ImageIcon(bi);

                Image img=foto.getImage();
                Image newimg=img.getScaledInstance(labelimagen.getWidth(), labelimagen.getHeight(), Image.SCALE_DEFAULT);
